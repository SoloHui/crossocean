...
# 公司名称
set(COMPANY_NAME "crossocean")
# 统一的命名空间
set(CROSSOCEAN_NAMESPACE "namespace crossocean {")
set(USING_CROSSOCEAN_NAMESPACE "using namespace crossocean;")
...
macro(set_cpp name)
  ...
  # 输出路径配置 Debug和Release配置同一个路径 默认为Debug
  ...
  foreach(type IN LISTS CONF_TYPES)
    ...
    set_target_properties(
      ${name}
      PROPERTIES RUNTIME_OUTPUT_DIRECTORY${conf} ${RUNTIME_DIR} # dll  exe 执行程序
...
function(cpp_library name)
  ...
  # 生成c++代码，传递命名空间
  configure_file(${CMAKE_SOURCE_DIR}/cmake/crossocean.h.in
                 ${CMAKE_CURRENT_LIST_DIR}/include/crossocean.h)
  ...
  # 静态库和动态库传递不同的宏变量给c++ XLOG_STATIC XLOG_EXPORTS
  if(${NAME}_SHARED)
  ...
  # 库的安装配置 ${CMAKE_INSTALL_PREFIX} cmake -S . -B build -D
  # CMAKE_INSTALL_PREFIX=./out cmake --install build --prefix=out
  set(version 1.0)
  # 设置安装的头文件
  set_target_properties(${name} PROPERTIES PUBLIC_HEADER "${H_FILE_I}")
  install(
    TARGETS ${name}
    EXPORT ${name}
  ...
  # 支持find_package 生成并安装配置文件 xlogConfig.cmake
  install(
    EXPORT ${name}
